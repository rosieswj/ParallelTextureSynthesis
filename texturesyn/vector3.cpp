...
void Vector3::Set(double a_X, double a_Y, double a_Z)
{
    x = a_X;
    y = a_Y;
    z = a_Z;
...
double Vector3::Red() const
{
    return x;
}
double Vector3::Blue() const
{
    return y;
}
double Vector3::Green() const
{
    return z;
}
...
void Vector3::operator+=(Vector3 *a_V)
{
    x += a_V->x;
    y += a_V->y;
    z += a_V->z;
